...
% Copyright (c) 2000-2014 by Tomlab Optimization Inc., $Release: 7.10.0$
% Written Sept 16, 2000.  Last modified Jan 8, 2014.
...
Result.x_k        = x;
Result.x_0        = x_0;
% Correct cLamda for Tomlab, change sign if iState = 2
Result.v_k        = cLamda;
ix                = find(iState == 2);
if ~isempty(ix)
   Result.v_k(ix) = -Result.v_k(ix);
end
...
% Could NOT use Result.v_k instead
...
% 140108 hkh Change sign v_k if iState 2 for Tomlab, but not SOL.cLamda
